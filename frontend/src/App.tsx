...
function loadProtocolFromStorage(): Protocol | null {
...
function frequencyToHz(frequency: string): number {
...
function App() {
  ...
  const [tetraConfig, setTetraConfig] =
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/rf/status")
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `HTTP ${response.status}`
  ...
  const buildStartPayload = () => {
    ...
    if (activeProtocol === "fm") {
      return {
        ...
        pre_emphasis:
          fmConfig.preEmphasis === "on",
      };
    }


    if (activeProtocol === "dmr") {
      return {
        protocol: "dmr",
  ...
  const handleStart = async () => {
    try {
      ...
      const response =
        await fetch(
          "http://localhost:8000/api/rf/start",
          {
            method: "POST",
  ...
  const handleStop = async () => {
    try {
      ...
      const response =
        await fetch(
          "http://localhost:8000/api/rf/stop",
          {
            method: "POST",
          }
        );
